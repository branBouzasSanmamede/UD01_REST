...
class ServicioManager:
    ...
    def cargar_servicios(self):
        ...
        for servicio in self.c.Win32_Service():
            servicios.append(Servicio(servicio.Name, servicio.ProcessId if servicio.ProcessId != 0 else None, servicio.State, servicio.StartMode, servicio.Description))
        return servicios
    ...
    def obtener_nombre(self):
        return input("Introduce el nombre del servicio: ").strip()
    ...
    def mostrar_desc(self):
        ...
        servicio = next((s for s in self.servicios if s.nombre == nombre), None)

        if not servicio:
            print(f"❌ No existe ningún servicio con el nombre: {nombre}")
            return
        
        mostrar_encabezado("Información del servicio")
